...
for episode in range(num_episodes):
    ...
    while not done:
        ...
        action_numpy = action_tensor.cpu().detach().numpy() * env.action_space.high[0]
        next_obs, reward, terminated, truncated, _ = env.step(action_numpy)
        
        agent.replayMemoryBuffer.store(obs, action_numpy, reward, next_obs, terminated)
        obs = next_obs
        ep_reward += reward
    ...
    print(f"Episode {episode}, Reward {ep_reward:.2f}, Total Timesteps: {total_time_steps}")
